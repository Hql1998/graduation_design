...
from knn_classifier_dialog import KNN_Classifier_Dialog
from function_widget import Function_Widget
import copy

class KNN_Classifier_Function_Widget(Function_Widget):

    def __init__(self, parent=None, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.class_name = "knn_classifier"
        ...
        self.data = None
        print("knn object name", self.objectName())
    ...
    def icon_btn_double_clicked_handler(self):
        ...
        if self.previous_widgets != [] and self.data is not None:
            if self.first_click:
                self.connected_dialog = KNN_Classifier_Dialog(self)
        ...
        print("KNN Classifier ", self.next_widgets, self.previous_widgets)
    ...
    def update_data_from_previous(self):
        if self.previous_widgets != [] and self.previous_widgets[0].data is not None:
            if self.data is None:
                ...
                print("previous", self.previous_widgets[0].data["train_x"].shape, "knn", self.data["train_x"].shape)
                self.data = copy.deepcopy(self.previous_widgets[0].data)
                self.connected_dialog.data = self.data
                self.connected_dialog.get_y_labels()
...
if __name__ == "__main__":
    ...
    window = KNN_Classifier_Function_Widget()
    window.show()
    sys.exit(app.exec_())